...
% Get decoding parameters :scale,quantization vector ,run-length table size
% ,zero run code ,end-of-data code ,wavelet bookkeeping array ,and
% run-length table .
n = double(y.n);
q = double(y.q) / 100;
runs = double(y.runs);
...
% Preform Huffman decoding followed by zero run decoding .
