...
%IM2JPEG2K Copresses an image using a JPEG 2000 approximation .
% y = IM2JPEG2K(X,N,Q) compresses image X using an N-scale JPEG 2K wavelet
% transform ,implicit or explicit coefficient quantization ,and Huffman
% symbol coding qugmented by zero run-length coding .If quantization vector
% Q contains two elements, they are assumed to be implicit quantization
% parameters ;else ,it is assumed to contain explicit subband step size.Y
% is an encoding structure containing Huffman-encoded data and additional
% parameters needed by JPEG2K2IM for decoding.
% 
% See also JPEG2K2IM.
global RUNS
...
% Find the run transition points : 'plus' contains the index of the start
% of a zero run ; the corresponding 'minus' is its end + 1.
...
%Find a zero run in the run-length table . If not found , create a new
%entry in the table. return the index of the run.
...
% decomposition (first to last ) and subband
% (horizontal,vertical,diagonal,and for final decomposition the
% approximation subband).
if length(p) == 2        % Iplicit Quantization
    q = [];
