...
%TIFS2CV Compresses a multiframe TIFF image sequence.
% Y = TIFS2CV(F,M,D,Q) compresses multiframe tiff f using motion
% compensated frames ,8 * 8 DCT transforms,and Huffman endcoding is used
% and the compression is lossless ;for Q > 0,lossy JPEG encoding is
% performed.The input are:
% 
%    F     A multiframe TIFF file       (e.g.,'file.tif')
...
% Verify that image dimension are multiples of the macroblock size.
...
% For all frames except the first,compute motion conpensated prediction
% residuals and compress with motion vectors.
for i = 2:fcnt
    ...
    %Code teh prediction residual and reconstruct it for use in forming the
    ...
    %Decode the next reference frame.Use the motion vectors to get the
    %subimages needed to subtract from the prediction residual.
